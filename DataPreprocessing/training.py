...
from DiffusionBase.DfTraining import train_next_token_diffusion, rolling_next_token_prediction, \
    predict_with_random_last_noise, predict_with_random_last_noise_2
...
def show_plot_for_diffusion():
    ...
    model = model.to(device)
    model.load_state_dict(torch.load("./trained_models/diffusion/model/H1.pth", map_location=device))
...
def show_plot_for_LT(m):
    ...
    if m == "LSTM":
        ...
        model = model.to(device)
        model.load_state_dict(torch.load("./trained_models/lstm/model/H1.pth", map_location=device))
    else:
        model = TransformerRegressor(input_dim, hidden_dim, input_dim)
        model = model.to(device)
        model.load_state_dict(torch.load("./trained_models/transformer/model/H1.pth", map_location=device))
