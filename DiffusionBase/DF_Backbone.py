...
class DFBackbone(nn.Module):
    ...
    def forward(self, z_t_prev, x_t_noisy, k, alpha_bar):
        k_float = k.float().unsqueeze(-1)
        normalized_k = k_float / 1000.0
        k_feature = normalized_k.repeat(1, 1, x_t_noisy.size(-1))

        x_concat = torch.cat([x_t_noisy, k_feature], dim=-1)
        input_epsilon_pred = torch.cat([z_t_prev, x_concat], dim=-1)
