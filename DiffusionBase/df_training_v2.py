...
#             optimizer.zero_grad()
#
#             k_min, k_max = get_scheduled_k(epoch, total_epochs, K)
...
# ###################MODEL EVAL###################
#
#                 val_loss += custom_loss(epsilon_pred, epsilon_true, xt_pred, x0,
#                                         kt, alpha_bar, epoch, total_epochs, loss_type)
#
...
#             xt = model.encoder(x0)
#
#             kt = torch.full((xt.shape[0], xt.shape[1]), random.randint(100, K-1), device=device)
#
#             xt_noisy = forward_diffuse(xt, kt, alpha_bar)
#
#             epsilon_true = compute_epsilon_true(xt_noisy, xt, kt, alpha_bar)
...
def df_training(model, data, validation_data, alpha_bar, K, total_epochs, scaler, loss_type, device, save_path):
    ...
    for epoch in range(total_epochs):
        ...
        for trajectory in data:
            x0 = trajectory.unsqueeze(0).to(device)
            xt = model.encoder(x0)
            # print("Before clamp - xt_pred min:", xt.min().item(), "max:", xt.max().item())
            if xt.min().item() < min:
                min = xt.min().item()
            if xt.max().item() > max:
                max = xt.max().item()
...
###################MODEL EVAL###################
                ...
                val_loss += custom_loss(epsilon_pred, epsilon_true, xt_pred, x0, kt, alpha_bar, epoch, total_epochs, loss_type)
...
def predict(model, test_data, alpha, alpha_bar, K, scaler, device):
    ...
    with torch.no_grad():
        ...
        for trajectory in test_data:
            x0 = trajectory.unsqueeze(0).to(device)
            xt = model.encoder(x0)
            kt = torch.full((xt.shape[0], xt.shape[1]), random.randrange(0, K-1)).to(device)
            ...
            xt_pred_full = xt_pred.squeeze(0).cpu().numpy()
            xt_true_full = x0.squeeze(0).cpu().numpy()
            predictions.append((xt_true_full,
                                xt_pred_full,
...
def predict_with_uncertainty(model, test_data, alpha, alpha_bar, K, scaler, device, T=30):
    ...
    with torch.no_grad():
        ...
        for i, trajectory in enumerate(test_data):
            x0 = trajectory.unsqueeze(0).to(device)
            xt = model.encoder(x0)
            kt = torch.full((xt.shape[0], xt.shape[1]), random.randrange(0, K-1), device=device)
            xt_noisy = forward_diffuse(xt, kt, alpha_bar)
